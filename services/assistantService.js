...
import { determineUserIntent, changeUserIntentFromSystem } from './userIntentService.js'
...
async function processUserInput(userInput, pv_user_data) {
    ...
    console.log("[INFO] process user input after intent:", pv_user_data);
    ...
    console.log("[INFO] user data after handle intent: ", updated_user_data);
    // pv_user_data["intent"] = pv_user_data

    const question = await createNextQuestion(pv_user_data);
    answer = answer + " " + question;
    updated_user_data = await changeUserIntentFromSystem(answer, pv_user_data); // TOFIX: if or_intent = '' => dodac or_intent
...
async function createNextQuestion(pv_user_data) {
    const content = 'Задай питання щоб визначити значення у намірі користувача, що записаний в original_intent.' //. Якщо original_intent є пустий рядок, тоді задай 1 питання до користувача щодо СЕС якщо залишилися незаповнені поля на обʼєкті.
...
function rewritePVUserData(pv_user_data, value, property) {
